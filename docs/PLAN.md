...
**16 Sep 2026, Session C step 4 (ATS detection).** `scripts/ats-detect.mjs` run over the 528 London / UK rows with a domain: 192 rows got an ATS (36%), 168 with a feed-verified slug (31%), 3,928 open roles behind those feeds. Ashby 73, Workable 36, Greenhouse 30, Teamtailor 15, other / custom 21, Lever 4, Personio 4, Recruitee 3, Workday 3, BambooHR 2, SmartRecruiters 1. Of the 336 misses, 47 sites refused or timed out and 105 had no careers page at all (mostly dormant seed-era names); among reachable companies with a careers page the hit rate is 51%, short of the brief's 60% and honest about why: in-house careers sites, HR-tech firms on their own ATS, and companies with nothing open. Step 3 (domain resolution for the 974 name-only rows, then HQ) is blocked on a search API key; once domains land, rerunning coverage and ATS detection picks them up.
