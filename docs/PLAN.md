...
**16 Sep 2026, Session C step 2 (coverage expansion, `docs/BRIEF-coverage-expansion.md`).** Startup Universe reloaded by hand with 2,590 rows (the seed minus 810 non-UK; it lives in the Employers base `app4AILlddDnxgRpq`, not the Sweeper base, because it links to Employers). New Worker endpoint `/companies?host=` returns a board's full company list: Getro through its collections search API (network id from the page's `__NEXT_DATA__`, `hits_per_page` up to 5000, no auth), Consider through `search-companies` on the jobs session. All 35 boards answer in 1 to 4 s; 8,237 company rows, 7,369 unique domains. Every company comes with domain and office locations, and Consider adds the ATS it posts from. `scripts/coverage.mjs` matches Universe rows on domain then normalised name and writes Boards, Board coverage, and blank Domain / HQ / London status. Dry run against the CSV mirror: 1,220 rows on a scraped board (was 335), 499 investor-board-not-listed, 871 investor-has-no-board, 989 domains and 1,034 HQs fillable. The write needs an Airtable token in `.env`. Found on the way: Creandum's board had moved from Consider to Getro and had been returning zero listings since at least 3 Sep; allowlist and Sources row switched, 26 recent London GTM roles visible again.

Coverage pass applied the same day: 1,152 rows on a scraped board, 531 investor board scraped but not listed, 907 investor has no board; 995 domains and 1,036 HQs filled from the boards; a rerun is a no-op.

**16 Sep 2026, Session C step 4 (ATS detection).** `scripts/ats-detect.mjs` run over the 528 London / UK rows with a domain: 192 rows got an ATS (36%), 168 with a feed-verified slug (31%), 3,928 open roles behind those feeds. Ashby 73, Workable 36, Greenhouse 30, Teamtailor 15, other / custom 21, Lever 4, Personio 4, Recruitee 3, Workday 3, BambooHR 2, SmartRecruiters 1. Of the 336 misses, 47 sites refused or timed out and 105 had no careers page at all (mostly dormant seed-era names); among reachable companies with a careers page the hit rate is 51%, short of the brief's 60% and honest about why: in-house careers sites, HR-tech firms on their own ATS, and companies with nothing open. A second pass over the other 1,088 rows with a domain (London status Unknown or non-UK office) named an ATS on 302 (27%), 241 feed-verified, 3,679 open roles; 97 unreachable, 322 with no careers page. Table totals: 494 rows with an ATS, 409 with a verified slug, about 7,600 open roles reachable through public feeds. **16 Sep 2026, Session C step 3 (domain resolution).** 974 name-only rows. Web search through parallel subagents applying the brief's rule (official site, title contains the name) resolved 299 before the session's shared search budget ran out; 326 domains written after a liveness check (some batches had already found extras), 20 proposed domains were dead, 141 searched and not found. SerpApi (`scripts/resolve-domains.mjs`, free tier 250/month, 234 spent) added 30 more where the name was in the domain itself; title-only matches turned out to be news and portfolio pages and were recorded as unresolved rather than written. Net: 661 rows still have no domain, each with a dated Notes line saying what was tried, or nothing if never searched (about 440). Rerunning coverage and ATS on the new domains: 1,216 rows on a scraped board, 553 rows with an ATS slug, roughly 8,000 open roles behind verified feeds. Next month's SerpApi quota, or one paid month, finishes the remainder; the scripts are rerunnable and skip rows already handled.

**16 Sep 2026, Session D (per-company ATS poller).** Worker 2.2.0 adds `/ashby`, `/greenhouse`, `/lever`, `/workable`, `/teamtailor`, `/recruitee`: one GET to the ATS's public feed, every open role mapped to the contract, term match on the title (whole word), then the shared filters; `source` is the company name n8n passes, `link` the ATS's own job URL. Greenhouse and Lever fall back to their EU APIs. Fixtures from six live feeds, 13 tests. The n8n workflow gained a parallel branch: read Startup Universe where `Poll` is ticked and `ATS slug` is set (Employers base, same PAT), build the endpoint per ATS, call it 25 at a time, write `Last polled` / `Last error` / `Listings pulled (last poll)` back, and merge into the same run summary, upsert and email. Only board failures trip the abnormal-volume guard; failing polls are listed in the email under their own heading. Rows with no Sources link are poller rows, and the email's Source line shows the company, so job-sweep labels them `VC Boards (<company>)` with no extractor change. Poll ticked on the 166 London / UK rows with a supported ATS and a verified slug (408 across the table qualify; the rest wait for HQ enrichment). The growth / PE and vendor names in the brief (Nscale, ClearScore, CUBE, Hostaway, Clay, Apollo, HubSpot, Cognism, n8n and so on) are not in Startup Universe yet and need adding by hand before they can be polled; Attest is there but on an unrecognised ATS. First live run 16 Sep 2026 (manual, 12:01 London): 166 companies polled in about 25 s, 152 answered, 14 Workable slugs came back 429 from a 25-call burst (now 8 calls per 1.5 s in n8n, plus two slow retries on 429 in the Worker); 9 on-lane roles inside the 7-day window at 6 companies, 6 of them new to Raw Listings and emailed with their canonical ATS links (Preply, Funding Circle, Anthropic, Frontier Health), the other 3 already known from Consider boards that publish the same ATS URL, so the link dedupe holds across boards and polls. Done criterion met. n8n note: an execution started through the MCP shows no node data while running and can look stuck; the scheduled run takes 6 to 8 minutes because of the per-row Airtable stamps.

**16 Sep 2026, Session E (keyword sources).** Worker 2.3.0 adds `/adzuna?q=`, `/reed?q=`, `/workable-search?q=` and `/rss?feed=revopscareers|clay`. Each keyword call keeps listings whose title contains the phrase or a config term, then the shared filters. The n8n sweep gained a third branch: Keyword Sources rows that are Active, not email alerts and have a Worker endpoint; one call per Keywords line (lines in brackets are notes), results grouped per row, `Last run` / `Last error` / `Listings pulled (last run)` written back, listings merged into the same upsert and email. Keyword errors are listed and never trip the guard. The email's Source line now comes from the run for every listing, so polled and keyword rows name their real source. Tested with pinned data in n8n (no writes, no email) and published. Live pilots on production: Jobs by Workable found a London GTM Engineer and a Revenue Operations Manager at Cledara; RevOps Careers 3 UK ops roles; Clay community 9 recent posts, noisy (mostly remote offshore agency roles; job-sweep's gates handle them). Departures from the brief: RevOps Careers' `/feed/` is its blog, so the endpoint uses the WP Job Manager `job_feed` filtered to United Kingdom, one call a day with no keyword because the site takes 11 to 22 s to answer; `/rss` takes an allowlisted `feed=` name rather than an arbitrary `url=`; `/gjobs` (SerpApi Google Jobs) is not built because this month's SerpApi quota is spent. Active now: Workable search, RevOps Careers, Clay. Adzuna and Reed have endpoints on their rows but stay inactive: the first attempt at the Adzuna secrets stored the key values as secret names (deleted), and the Reed key has not arrived. `/healthz` reports which key secrets are set, as booleans.

**16 Sep 2026, UK presence from job feeds.** `scripts/hq-from-feeds.mjs` read the verified ATS feed of the 242 companies whose UK status was unknown: 85 are hiring in London, 23 elsewhere in the UK, 134 only abroad (dated note on each, status left unchanged). Poll ticked on the 108 new London / UK rows: 274 companies polled from the next run. A Companies House name match was piloted first and rejected for this job (wrong company on common names, an Irish-registered match, a London match for a firm hiring only in Porto); the script stays as a review-by-hand tool. The run is Monday to Friday at 01:00 London (README had 06:30). Open question: the workflow still deletes Raw Listings first seen over 30 days ago although the paid plan makes that unnecessary.

**17 Sep 2026, backlog and audit targets.** Open work now lives in a `Tasks` table in the VC Job Sweeper base (`tbliXgrrxB88ZALW2`), modelled on the Home OS one with an added Owner field. The first run with all three branches (17 Sep 01:00) polled every company with no errors, so the Workable 429 fix holds; keyword sources reported in. The GTMfund reference row in Keyword Sources had a Worker endpoint and was being called as a keyword source on top of its board; endpoint cleared. Added the coverage audit's 13 named targets missing from Startup Universe with live-checked domains (Unity Advisory is `unity-advisory.com`; Prevalent now sits under `mitratech.com`); ATS found for 12 of 14 including Attest, whose slug was wrong. Polled now: Clay, Apollo, HubSpot, Cognism, n8n, CUBE, Unity Advisory, Attest, Maze, Prevalent/Mitratech. Hostaway and Trackunit have no UK roles today; Nscale and ClearScore have no readable feed. Two Teamtailor hosts allowlisted (`careers.askattest.com`, `careers.trackunit.com`). `hq-from-feeds.mjs` fixes: London, Ontario written as "London, Canada" no longer counts as London (Trackunit reverted), and rows already checked are skipped unless `--recheck`. Found 31 duplicate company rows polling the same feed (the seed has one row per fund listing); Poll turned off on the second copy with a note, so the nightly run polls 253 distinct companies, not the 274 rows it counted before. Merging the duplicate rows is a backlog task. A `Domain lookup` formula field was added to Startup Universe for the AI assist fix.

**17 Sep 2026, duplicate rows merged.** `scripts/dedupe-universe.mjs` grouped Startup Universe rows sharing a domain, or sharing a normalised name where one had no domain or both domains had the same label (causaly.com / causaly.ai); two rows with different verified job feeds are never merged (Kraken the exchange and Kraken Technologies). 134 groups merged: the most useful row kept, blanks filled, Investors, Boards, Source and Employers links combined, notes carried over; 135 rows deleted, 34 of them name-only. Every affected row is backed up in `docs/data/startup-universe-merged-2026-09-17.json`. Table now 2,468 companies, 1,841 with a domain, 627 without, 253 polled, no feed polled twice; a rerun finds nothing.

**17 to 20 Sep 2026, enrichment engine (`scripts/enrich.mjs`, skill `enrich`).** First design had one subagent research each row with web search: about 8,000 tokens and 70 seconds a row, capped at about 200 searches a session, a week of scheduled Cowork sessions for the table. Rejected and replaced. Now code gathers evidence for every row in parallel (`scripts/lib/gather.mjs`: homepage title and description, name-based domain guesses plus Clearbit autocomplete, links on the company's own homepage), subagents with no web tools judge compact evidence files in bulk only where judgement is needed, and `apply` routes by confidence: high into the field, medium and low into a `<X> candidate` column with `<X> confidence` for Tim to approve (`promote` copies approved ones), nothing found marked `Couldn't fetch`. State lives in Airtable (the recipe's filter), not in files. Results: `Domain check` on all 1,904 domains (1,623 Confirmed, 70 Suspect, mostly acquisitions and rebrands, 200 Unclear, 11 Wrong: parked or hijacked) in about 25 minutes; of 482 rows with no domain, 64 written at high confidence, 170 candidates for review, 248 could not be fetched (many dead or acquired; web search for that residue is parked). `linkedin` recipe needs no model: 15-row pilot found 9 in 3 seconds. `scripts/lib/airtable.mjs` now times out and retries, after the API hung for five minutes mid-run. Context Scout's domain cleared (unrelated site) and KRY corrected from "livi" to kry.co.uk.
